...
function buildGrid(xmin: number, xmax: number, ymin: number, ymax: number, n: number): GridResult {
  const X: number[][] = []
  const Y: number[][] = []
  const Z: (number | null)[][] = []
...
function evaluateGrid(expr: string, xmin: number, xmax: number, ymin: number, ymax: number, n = 60): GridResult {
...
export default function DomainRangeCalculator() {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <h3 className="text-base font-semibold">Dominio, rango y mapa de validez</h3>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
            ...
            <div>
              ...
              <Input id="ymin" value={ymin} onChange={e => setYmin(e.target.value)} />
            </div>
            <div>
              <Label htmlFor="ymax">y max</Label>
              <Input id="ymax" value={ymax} onChange={e => setYmax(e.target.value)} />
            </div>
            <div>
              <Label htmlFor="n">Resolución (10–200)</Label>
              <Input id="n" value={n} onChange={e => setN(e.target.value)} />
            </div>
            <div className="md:col-span-2">
              <Label htmlFor="expr">f(x,y)</Label>
              <Input id="expr" value={expr} onChange={e => setExpr(e.target.value)} />
            </div>
          </div>

          {!result && (
            <div className="text-xs text-gray-400">Ajusta los parámetros para ver el dominio muestreado.</div>
          )}
          ...
          {result && (
            <div className="text-xs text-gray-300">
              <div>celdas válidas: {result.validCount} / {result.totalCount}</div>
              <div>
                rango z:{" "}
                {result.zMin === null ? "—" : result.zMin.toFixed(4)}{" "}
                a{" "}
          ...
          <Button type="button" onClick={() => { /* solo para mantener un CTA */ }}>
            Recalcular
          </Button>
        </CardContent>
      </Card>
