...
function integrateBox(
  ...
  const [xa, xb] = xr, [ya, yb] = yr, [za, zb] = zr
  const dx = (xb - xa) / nx
  const dy = (yb - ya) / ny
  const dz = (zb - za) / nz
  let sum = 0, valid = 0
...
function centerOfMassBox(
  ...
  const dy = (yb - ya) / ny
  const dz = (zb - za) / nz
  let M = 0, Mx = 0, My = 0, Mz = 0, valid = 0
...
export default function IntegrationTriple() {
  ...
  const runIntegral = () => {
    ...
    setOutInt(`∭_Box f dV ≈ ${value.toFixed(6)}  (válidas: ${valid}/${samples}, N=${N})`)
  }
  ...
  const runCenterOfMass = () => {
    ...
    const { M, xbar, ybar, zbar, samples, valid } = centerOfMassBox(rho, xr, yr, zr, N, N, N)
    const pos = `(${Number(xbar).toFixed(6)}, ${Number(ybar).toFixed(6)}, ${Number(zbar).toFixed(6)})`
    setOutCM(`M ≈ ${M.toFixed(6)};  centro ≈ ${pos}  (válidas: ${valid}/${samples}, N=${N})`)
  }

  return (
    <div className="space-y-6">
      <Card>
        <CardHeader><CardTitle>Integración triple (caja)</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <div className="space-y-2">
            <Label>f(x,y,z)</Label>
            <Input value={f} onChange={e=>setF(e.target.value)} placeholder="x^2 + y^2 + z^2" />
          </div>

          <div className="grid grid-cols-3 gap-2">
            <div><Label>x mín</Label><Input value={xa} onChange={e=>setXa(e.target.value)} /></div>
            <div><Label>y mín</Label><Input value={ya} onChange={e=>setYa(e.target.value)} /></div>
            <div><Label>z mín</Label><Input value={za} onChange={e=>setZa(e.target.value)} /></div>
            <div><Label>x máx</Label><Input value={xb} onChange={e=>setXb(e.target.value)} /></div>
            <div><Label>y máx</Label><Input value={yb} onChange={e=>setYb(e.target.value)} /></div>
            <div><Label>z máx</Label><Input value={zb} onChange={e=>setZb(e.target.value)} /></div>
          </div>
          ...
          <Button className="w-full" onClick={runIntegral}>Integrar ∭</Button>
          {outInt && <div className="text-sm bg-gray-900 rounded p-3">{outInt}</div>}
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle>Centro de masa (densidad ρ)</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <div className="space-y-2">
            <Label>ρ(x,y,z)</Label>
            <Input value={rho} onChange={e=>setRho(e.target.value)} placeholder="1" />
          </div>

          <Button className="w-full" onClick={runCenterOfMass}>Calcular centro de masa</Button>
