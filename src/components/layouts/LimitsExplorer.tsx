 ...
 *
 * Dependencia: mathjs
 *   pnpm add mathjs   |   yarn add mathjs   |   npm i mathjs
 */
...
export default function LimitsExplorer({ functionExpr, onPointChange }: LimitsExplorerProps) {
  ...
  const result = useMemo(() => {
    ...
    for (const p of all) {
      for (const pt of p.points) {
        const v = evalAt(compiled, pt.x, pt.y);
        p.values.push(v);
      }
      const finiteSeq = p.values.filter((v) => Number.isFinite(v));
      if (finiteSeq.length >= tail) {
        const an = analyzeConvergence(finiteSeq, tail);
        if (an.ok && Number.isFinite(an.mean) && an.maxDev < tol) {
          p.convergesTo = an.mean;
        }
      }
    }

    const verdict = globalVerdict(all, tol);
    return { radii, paths: all, verdict } as const;
  ...
  const probe = useMemo(
    () => epsilonDeltaProbe(compiled, x0, y0, epsilon, delta),
    [compiled, x0, y0, epsilon, delta]
  );
  ...
  return (
    <div className="w-full grid gap-4 md:grid-cols-2">
        ...
        <Tabs defaultValue="paths" className="mt-2">
          ...
          <TabsContent value="paths" className="space-y-3 mt-3">
            <div>
              ...
              <Slider
                value={[levels]}
                onValueChange={(val: number[]) => setLevels(Math.round(val[0]))}
                min={3}
                max={12}
                step={1}
              />
            ...
            <div>
              ...
              <Slider
                value={[base]}
                onValueChange={(val: number[]) => setBase(Number(val[0].toFixed(2)))}
                min={0.05}
                max={0.5}
                step={0.01}
              />
            ...
            <div>
              ...
              <Slider
                value={[thetaCount]}
                onValueChange={(val: number[]) => setThetaCount(Math.round(val[0]))}
                min={4}
                max={24}
                step={1}
              />
            ...
            <div>
              ...
              <Slider
                value={[randCount]}
                onValueChange={(val: number[]) => setRandCount(Math.round(val[0]))}
                min={0}
                max={6}
                step={1}
              />
          ...
          <TabsContent value="tolerances" className="space-y-3 mt-3">
            <div>
              ...
              <Slider
                value={[tail]}
                onValueChange={(val: number[]) => setTail(Math.round(val[0]))}
                min={2}
                max={8}
                step={1}
              />
              <div className="text-xs text-muted-foreground">
                Se observan los últimos {tail} valores por ruta
              </div>
            </div>
            <div>
              ...
              />
              <div className="text-xs text-muted-foreground">
                Si la dispersión ≤ tolerancia, aceptamos mismo límite
              </div>
            </div>
          </TabsContent>
        </Tabs>
        ...
        <div className="text-sm mt-1">
          {probe.ok ? (
            <span className="text-green-600">
              Para este δ, la variación máxima (~{probe.maxVar.toExponential(2)}) está por debajo de ε.
            </span>
          ) : (
            <span className="text-amber-600">
              Para este δ, la variación máxima (~{probe.maxVar.toExponential(2)}) supera ε.
            </span>
          )}
        </div>
      </Card>
      ...
      <Card className="p-4 space-y-4">
                  ...
                  return (
                    <tr key={i} className="border-t">
                      ...
                      <td className="p-2 align-top font-mono text-xs">
                        {tailVals.length ? (
                          tailVals.map((v) => Number(v).toExponential(2)).join(", ")
                        ) : (
                          <span className="text-muted-foreground">n/a</span>
                        )}
                      </td>
                      <td className="p-2 align-top">
                        {Number.isFinite(p.convergesTo ?? NaN) ? (
                          Number(p.convergesTo).toPrecision(6)
                        ) : (
        ...
        <div className="text-xs text-muted-foreground">
          Nota: Este módulo usa verificación <strong>numérica</strong> por múltiples rutas. Si detecta rutas con
          límites distintos, reporta contraejemplo. Si todas coinciden dentro de la tolerancia, sugiere un valor
          candidato.
        </div>
      </Card>
    </div>
  ...
  if (verdict.status === "exists") {
    return (
      <div className="rounded-xl border p-3 bg-emerald-50">
        ...
        <div className="text-xs text-muted-foreground">
          Dispersión máxima entre rutas: {verdict.spread?.toExponential(2)}
        </div>
      </div>
    );
  }
      ...
      <ul className="list-disc pl-6 text-sm mt-1">
        {verdict.pairs?.slice(0, 6).map((p, i) => (
          <li key={i}>
            ...
            {Number.isFinite(p.value) ? Number(p.value).toPrecision(6) : "n/a"}
          </li>
        ))}
      </ul>
      <div className="text-xs text-muted-foreground mt-2">
        Si alguna ruta es inestable, intenta reducir la base geométrica o aumentar niveles.
      </div>
    </div>
  );
}
